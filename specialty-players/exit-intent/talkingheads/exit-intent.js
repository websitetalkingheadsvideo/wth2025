...
jQuery.fn.extend({
	talkingPopup: function () {
		...
		$("<style type='text/css'>#message{font-size:2rem}#column-2{padding-right:.5rem} #column-1{width:330px} #column-1,column-2{float:left;margin:2px}#row{max-width:760px;margin:0;clear:both} #exitpopup { text-align: center;width:" + popupWidth + "px; height:" + popupHeight + "px; margin:0px auto; display:none; position:fixed; color:#000000;-webkit-border-radius: 4px; -moz-border-radius: 8px; border-radius: 8px;border:2px solid #3D9CEB; z-index:999999; background:rgb(20, 20, 20); background:#FFFFFF; } #popup_title { margin-top: 0px; padding-top: 0px; } #exitpopup p { text-align: left; } #exitpopup_bg{display: none; width:100%; height:100%; position:fixed; background:#000000; opacity: .8; filter:alpha(opacity=0.8); z-index:999998;}#popupClose{margin-left: " + exitLeft + "px;}</style>").appendTo("head");
			...
			if (e.pageY <= 5) {
				if (!up) {
					...
					if (moveTime < 3) {
						...
						$('#message').text("Videos grab an audience's attention more effectively than other medium. It is quick, succinct, and it engages an individual's psychological professors and physical senses to a higher degree, therefore keeping Web users inhabited. Exactly what's likewise excellent about this is that individuals can view the videos anywhere, on their laptop computers, iPads, and their smart phones.");
					} else if (moveTime < 10) {
						$('#reporter').text('3-10 Second Message ');
						$('#message').text('Our Online Spokesperson features the most robust, totally free, copyrighted gamer on the marketplace today. It utilizes HTML5 to put our spokesperson on your site with a transparent background. Nobody has more than 30 variable functions.');
					} else {
						$('#reporter').text('More than 10 Seconds Message ');
						$('#message').text('Our Virtual Actor may convey details, give headlines updates, give information or even guide guests around your web site, their project might be actually to keep your visitors focus. Then entice them to buy your Product or Service. Certainly not a simple accomplishment.');
					}
		...
		var width = "320", //video width
			...
			popPlayer, hVideo, theParent, actorGif, clickImage, talkingHead, toLoop, toMute = !1;
