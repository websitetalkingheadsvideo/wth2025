...
$(document).ready(function () {
    ...
    // Scroll behavior - scroll position directly drives header animation
    var scrollThreshold = 100; // pixels of scroll to reach full "scrolled" state
    ...
    $(window).on('scroll', function () {
        ...
        var scrollProgress = Math.min(1, scrollTop / scrollThreshold);

        // Apply scroll progress as CSS custom property for smooth, direct control
        root.style.setProperty('--scroll-progress', scrollProgress);

        // Add/remove scrolled class for CSS fallback
        if (scrollTop > 0) {
        ...
        actorCards.each(function() {
            ...
            // Dim if card's top half is under the header
    ...
    // Initialize video autoplay for actor card thumbnails
    $('.actor-card-thumbnail video').each(function() {
        var video = this;
        video.muted = true;
        ...
        if (playPromise !== undefined) {
            playPromise.catch(function(error) {
                // Autoplay was prevented, will try again on loadeddata
                console.log('Video autoplay initially prevented:', error);
            });
        }
    });

    // Split actor names into individual letter spans for animation
    ...
    $('.actor-card-name').each(function() {
        ...
        // Measure text width
        while ($name[0].scrollWidth > parentWidth && currentFontSize > minFontSize) {
    ...
    actorCards.on('mousemove', function(e) {
        ...
        if (letters.length > 0) {
            ...
            letters.each(function(i) {
                ...
                var maxLetterDist = 100;
                var letterIntensity = Math.max(0, 1 - (distToLetter / maxLetterDist));
                var letterScale = 1 + (letterIntensity * 0.20); // up to 120%
    ...
    actorCards.on('mouseenter', function() {
        ...
        var lingerInterval = setInterval(function() {
            ...
            if (time > 500) {
                var progress = Math.min(1, (time - 500) / 2000);

                // Apply unfocused effect to other cards
                actorCards.not(hoveredCard).each(function() {
                    $(this).addClass('unfocused');
                    var brightness = 1 - (progress * 0.3); // down to 0.7
                    var saturation = 1 - (progress * 0.5); // down to 0.5
                    var blur = progress * 2; // up to 2px
