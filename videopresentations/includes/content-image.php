...
if ( $result->num_rows > 0 ) {
	...
	$spintax = new Spintax();
	$alt = $spintax->process( '{Custom |}{Web|Internet|Website|Online}{ Marketing|} {Video|}' );
	echo PHP_EOL;
	while ( $row = $result->fetch_assoc() ) {
		...
		echo PHP_EOL;
		echo( '<div class="fifty pull-left">'  );
		echo PHP_EOL;
		echo '<img src="http://seovideoexperts.com/wp-content/uploads/wp_image_duplicator_uploads/' . $url . '.jpg" class="img-responsive" title="' . $alt. '" alt="' . $alt . '">';
		echo PHP_EOL;
		echo( '   </div>' );
		echo PHP_EOL;
		echo( '<div>' );
