...
<section class="alert bg-gradient-dark">
  ...
  echo "<h2 class='text-light'>Website Talking Heads</h2>
  <h1 class='text-light'>Site Map</h1>
  <div class='container'>
  ";
  $sitemap_url = "https://www.websitetalkingheads.com/sitemap.xml";
  $xml = simplexml_load_file( $sitemap_url );
  $array = json_decode( json_encode( simplexml_load_file( $sitemap_url ) ), TRUE );
  /*  print_r( $array );*/
  foreach ( $array[ 'url' ] as $url_data ) {
    echo "<a href=".$url_data[ 'loc' ] .">";
    echo $url_data[ 'loc' ];
    echo "</a><br>
	";
  }
