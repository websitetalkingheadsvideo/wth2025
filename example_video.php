...
<body>
	...
	$video = $_GET[ video ];
	$color = $_GET[ color ];
	$baseUrl = "https://s3-us-west-2.amazonaws.com/example-videos/";
	$poster = $baseUrl . $video . ".png";
	$theVideo = $baseUrl . $video . ".mp4";
	if ( empty( $color ) ) {
		$color = "FFF";
	}
	$color = "#" . $color;
	...
	<script>
		$( document ).ready( function () {
			...
			$( ".player-list" ).css( "background-color", color );
			$( "#clicktoplay" ).css({ "color": color, "border-color": color});
		} );
	</script>
	<div class="player" id="video-player">
		<video <?php if($autostart==="true"){echo 'autoplay ';} if($mute==="true"){echo 'loop muted';} ?> id="Talking_Heads_Video"  poster="<?=$poster?>" data-toggle="tooltip" title="<?=$video?>">
