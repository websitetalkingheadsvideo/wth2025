// JavaScript Document

function validateOrder(){
	//Set up reporting variables
	var r = true;
	var message = "";

//validate name
var cn=document.getElementById("nameText").value; 
if (cn == "" || cn=="null"  || cn =="Name"||  cn.length < 3 ) {
      r=false;
	  message += "Your Name.\n";
   	  document.getElementById("nameText").style.outline = "solid red";
    } else {
	   document.getElementById("nameText").style.outline = "none";
   }


//validate email and phone
var validateEmail = false;
var x=document.getElementById("emailText").value;

var atpos=x.indexOf("@");
var dotpos=x.lastIndexOf(".");

if (atpos<1 || dotpos<atpos+2 || dotpos+2>=x.length)
 {
   validateEmail=false;
   document.getElementById("emailText").style.outline = "solid red";
  }else {
	   document.getElementById("emailText").style.outline = "none";
	   validateEmail = true;
   }

var validatePhone = false;
var phoneNumber = document.getElementById("phoneNumber").value; 
  if (phoneNumber == ""  ||  phoneNumber == "Phone"|| phoneNumber == "123456"  || phoneNumber.length<10 || phoneNumber == null) {
		document.getElementById("phoneNumber").style.outline = "solid red";
		document.getElementById("phoneNumber").value = "";
     validatePhone = false;
   }else{
		document.getElementById("phoneNumber").style.outline = "none";
		var validatePhone = true;
}
//phone and email test
if(validatePhone == false && validateEmail == false){
	message +="A valid email address or phone number.\n";
      r=false;
}

//test terms and conditions acceptance
	var termsMet =document.getElementById("termsCheckbox").checked;
	if(termsMet){
				document.getElementById("termsCheckbox").style.outline = "none";
	}else{
				message +="You must accept our terms and conditions.\n";
				r=false;
				document.getElementById("termsCheckbox").style.outline = "solid red";
	}

	
//Final test
	if (r){
		return true;
		}else{
		var messageIntro= "";
		messageIntro+= "We would like to complete your order.\n";
		messageIntro+= "Please provide the following information.\n";
		var fullMessage = messageIntro + message;
		alert(fullMessage);
		return false;
		}
	}
